...
function run_two_stream(; use_gpu::Bool = false, save_every::Int = 10)
    ...
    simulation_name = "two_stream_filtered_v5"
    ...
    psi_mps = apply(itensor_mpos.half_filtering, psi_mps; alg = params.alg, maxdim = maxrank, cutoff = params.cutoff)
    ...
    for step in iter
        psi_mps, ef_mps = strang_step_filtered_RK4!(
            psi_mps,
            phase,
